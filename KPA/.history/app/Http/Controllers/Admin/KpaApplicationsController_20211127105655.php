...
use App\Http\Requests\StorepaApplicationRequest;
use App\Http\Requests\UpdatepaApplicationRequest;
use App\paApplication;
use App\Role;
...
class kpaApplicationsController extends Controller
{
    public function index()
    {
        abort_if(Gate::denies('kpa_application_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');

        $kpaApplications = paApplication::with('status', 'analyst', 'cfo')->get();
    ...
    public function store(StorepaApplicationRequest $request)
    {
        $kpaApplication = paApplication::create($request->only('kpa_amount', 'description'));

        return redirect()->route('admin.kpa-applications.index');
    }

    public function edit(paApplication $kpaApplication)
    {
        abort_if(
            Gate::denies('kpa_application_edit') || !in_array($kpaApplication->status_id, [6,7]),
    ...
    public function update(UpdatepaApplicationRequest $request, paApplication $kpaApplication)
    {
    ...
    public function show(paApplication $kpaApplication)
    {
        abort_if(Gate::denies('kpa_application_show'), Response::HTTP_FORBIDDEN, '403 Forbidden');
    ...
    public function destroy(paApplication $kpaApplication)
    {
        abort_if(Gate::denies('kpa_application_delete'), Response::HTTP_FORBIDDEN, '403 Forbidden');
    ...
    public function massDestroy(MassDestroypaApplicationRequest $request)
    {
        paApplication::whereIn('id', request('ids'))->delete();

        return response(null, Response::HTTP_NO_CONTENT);
    }

    public function showSend(paApplication $kpaApplication)
    {
    ...
    public function send(Request $request, paApplication $kpaApplication)
    {
        abort_if(!auth()->user()->is_admin, Response::HTTP_FORBIDDEN, '403 Forbidden');
    ...
    public function showAnalyze(paApplication $kpaApplication)
    {
        $user = auth()->user();
    ...
    public function analyze(Request $request, paApplication $kpaApplication)
    {
        $user = auth()->user();
