...
{
    public static function generateLogs(KpaApplication $kpaApplication)
    {
        $changes      = [];
        $statuses     = Status::pluck('name', 'id');
        $users        = User::pluck('name', 'id');

        foreach ($kpaApplication->logs as $log) {
                ...
                foreach ($differences as $key => $difference) {
                    ...
                    if ($key == 'status_id') {
                        ...
                        if (in_array($difference, [3, 4, 6, 7])) {
                            ...
                            $value['comment'] = $kpaApplication->comments->where('user_id', $current[$column])->first()->comment_text;
                        }
